      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        activityCard.innerHTML = `
          ...
          <div class="participants">
            ...
            <ul>
              ${details.participants.length > 0 ? details.participants.map((participant) => `<li class="participant-item"><span class="participant-email">${participant}</span><button type="button" class="participant-remove" data-activity="${name}" data-email="${participant}" aria-label="Remove participant ${participant}">✕</button></li>`).join("") : `<li class="no-participants">No participants yet</li>`}
            </ul>
          </div>
        `;
